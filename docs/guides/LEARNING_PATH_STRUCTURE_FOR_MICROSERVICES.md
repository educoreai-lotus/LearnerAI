...
## 📦 Learning Path Structure (`learning_path` field)
...
| `competency_target_name` | String | ✅ Yes | The competency/course name (e.g., "C++ basics Noam") |
| `learning_path` | Object (JSONB) | ✅ Yes | Complete learning path structure |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `pathTitle` | String | ⚠️ Optional | Title of the learning path |
| `totalDurationHours` | Number | ⚠️ Optional | Total estimated duration in hours |
| `learning_modules` | Array | ⚠️ Conditional | **Primary format** - Array of module objects (if module-based) |
| `steps` | Array | ⚠️ Conditional | **Legacy format** - Array of step objects (if step-based) |
| `estimatedCompletion` | String | ⚠️ Optional | Estimated completion time (for steps format) |
| `totalSteps` | Number | ⚠️ Optional | Total number of steps (for steps format) |
...
| `status` | String | ⚠️ Optional | Approval status: "approved", "pending", "rejected" |
| `metadata` | Object | ⚠️ Optional | Additional metadata (usually empty object `{}`) |
| `companyId` | String (UUID) | ⚠️ Optional | Company ID (duplicate of root level) |
| `competencyTargetName` | String | ⚠️ Optional | Competency name (duplicate of root level) |
...
| `module_title` | String | ✅ Yes | Title of the module |
| `estimated_duration_hours` | Number | ✅ Yes | Estimated duration for this module in hours |
| `skills_in_module` | Array of Strings | ✅ Yes | **New format** - List of lowest-level skills included in this module |
| `focus_micro_skills` | Array of Strings | ⚠️ Legacy | **Legacy format** - Alias for `skills_in_module` (for backward compatibility) |
| `learning_goals` | Array of Strings | ✅ Yes | Learning objectives/goals for this module |
| `steps` | Array of Step Objects | ✅ Conditional | **New format** - Array of structured step objects (if using new format) |
| `suggested_content_sequence` | Array of Strings | ⚠️ Legacy | **Legacy format** - Ordered list of content items as strings (for backward compatibility) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `step` | Number | ⚠️ Optional | Step number (0, 1, 2...) |
...
## 📝 Complete Example (New Format with Structured Steps)
